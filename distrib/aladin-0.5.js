 ...
 *****************************************************************************/
    ...
    Color.curIdx = 0;
    Color.colors = ['#ff0000', '#0000ff', '#000066', '#00ffff', '#9900cc', '#0099cc'];
 ...
 * File Overlay
 *
 * Description: a plane holding overlays (footprints for instance)
 * 
 * Author: Thomas Boch[CDS]
 * 
 *****************************************************************************/
...
cds.Source = (function() {
    // constructor
    cds.Source = function(ra, dec, data) {
    	this.ra = ra;
    	this.dec = dec;
    	this.data = data;
    	this.catalog = null;
...
HpxImageSurvey = (function() {
    ...
    var HpxImageSurvey = function(id, name, rootUrl, cooFrame, maxOrder) {
        this.id = id;
    	this.name = name;
    	this.rootUrl = rootUrl;
        if (cooFrame && cooFrame.toLowerCase()=="galactic") {
...
View = (function() {
    ...
    /** Constructor */
    function View (aladin, imageCanvas, catalogCanvas, reticleCanvas, location, cooFrame, zoom) {
            this.aladin = aladin;
...
//		}
	...
	redraw = function() {
		...
		// black background
        if (view.projectionMethod==ProjectionEnum.SIN) {
            if (view.fov>80) {
            ...
            // pour éviter les losanges blancs qui apparaissent quand les tuiles sont en attente de chargement
                ...
                imageCtx.fillStyle = "rgb(0,0,0)";
                imageCtx.fillRect(0, 0, view.imageCanvas.width, view.imageCanvas.height);
            }
        }
		...
		////// 3. Draw overlays////////
        var overlayCtx = view.catalogCtx;
		if (view.overlays && view.overlays.length>0 && (! view.dragging  || View.DRAW_SOURCES_WHILE_DRAGGING)) {
		    if (! catalogCanvasCleared) {
		        catalogCtx.clearRect(0, 0, view.width, view.height);
                catalogCanvasCleared = true;
		    }
		    for (var i=0; i<view.overlays.length; i++) {
		        view.overlays[i].draw(overlayCtx, view.projection, view.cooFrame, view.width, view.height, view.largestDim, view.zoomFactor, view.cooFrame);
		    }
        }
		...
		if (view.mustRedrawReticle || view.mode==View.SELECT) {
		...
		if (view.mode==View.SELECT && view.dragging) {
		    ...
		    var w = view.dragx - view.selectStartCoo.x;
		    var h =  view.dragy - view.selectStartCoo.y;
		    
		    reticleCtx.fillRect(view.selectStartCoo.x, view.selectStartCoo.y, w, h);
		}
...
//			}
	...
	View.prototype.setImageSurvey = function(imageSurvey) {
		...
		if (typeof imageSurvey == "string") {
		    var surveyInfo = HpxImageSurvey.getSurveyInfoFromId(imageSurvey);
		    if ( ! surveyInfo) {
		        surveyInfo = HpxImageSurvey.getSurveyInfoFromId(HpxImageSurvey.DEFAULT_SURVEY_ID);
		    }
		    this.imageSurvey = new HpxImageSurvey(surveyInfo.id, surveyInfo.name, surveyInfo.url, surveyInfo.frame, surveyInfo.maxOrder);
		}
		else {
	...
	var Aladin = function(aladinDiv, requestedOptions) {
	    ...
	    $.ajax({
	        ...
	        success: function(data) {
	            HpxImageSurvey.SURVEYS = data.sort(function(a, b) {return a.order > b.order ? 1 : -1;});
	            // màj liste déroulante
...
//		canvas.mozImageSmoothingEnabled = false;
		...
		// some CSS class declarations
		var style = "#zoomControl {z-index: 15;position:absolute; top: 50%; height: 48px; right: 8px; padding: 0; margin: -24px 0 0 0; font-weight: bold; font-size: 18px; font-family: Verdana, Lucida, Arial;}\n";
		style += '#zoomControl a {width: 20px;height:20px;line-height: 18px;display:block;background-color:rgba(250, 250, 250, 0.8);margin:1px;text-align:center;border-radius:4px;border:1px solid #aaa;text-decoration:none;color:#222;}\n';
		style += '#zoomControl a:hover {background-color: rgba(230, 230, 230, 0.8);}\n';
		$('<style type="text/css">' + style + '</style>').appendTo($(aladinDiv));
		
		// div where we write the position
		var locationDiv = $('<div id="locationDiv" style="z-index: 20;position:absolute; padding: 2px 4px 2px 4px;  font-size: 13px; background-color: rgba(255, 255, 255, 0.5)"></div>').appendTo(aladinDiv);
		
		// TODO : mettre tous les styles dans un CSS !
		
		// zoom control
        if (options.showZoomControl) {
	          $('<div id="zoomControl"><a href="#" id="zoomPlus" title="Zoom in">+</a><a href="#" id="zoomMinus" title="Zoom out">&ndash;</a></div>').appendTo(aladinDiv);
	    }
		...
		// Aladin logo
		$("<a href='http://aladin.u-strasbg.fr/AladinLite/' title='Powered by Aladin Lite' target='_blank'><img src='" + Aladin.LOGO + "' width='32' height='32' style='position: absolute; padding: 0; bottom: 5px; right: 8px; z-index: 3;' /></a>").appendTo(aladinDiv);
		...
		if (options.showControl) {
	        ...
	        '<select id="surveySelection" style="width: 200px;"></select><br/>' +
	        'Frame: <select id="frameChoice"><option selected="selected">J2000</option><option>GAL</option></select><br/>' +
			...
			$('#showControlBox').click(function() {$('#controlBox').show();return false;});
			$('#closeControlBox').click(function() {$('#controlBox').hide();return false;});
		}
		...
		this.view = new View(this, imageCanvas, catalogCanvas, reticleCanvas, location, cooFrame, options.zoom);
        this.gotoObject(options.target);

        if (options.log) {
            Logger.log("startup", requestedOptions);
        }
		...
		//var surveyInfo = HpxImageSurvey.getSurveyInfoFromId(options.survey);
		//this.setImageSurvey(new HpxImageSurvey(surveyInfo.id, surveyInfo.name, surveyInfo.url, surveyInfo.frame, surveyInfo.maxOrder));
        ...
        $('#surveySelection').change(function() {
        	...
        	aladin.setImageSurvey(new HpxImageSurvey(survey.id, survey.name, survey.url, survey.frame, survey.maxOrder));
        });
    ...
    Aladin.LOGO = "data:image/gif;base64,";
    ...
    Aladin.prototype.updateSurveysDropdownList = function(surveys) {
        ...
        for (var i=0; i<surveys.length; i++) {
            ...
            $('#surveySelection').append($("<option />").attr("selected", isCurSurvey).val(surveys[i].id).text(surveys[i].name));
        };
    };
    ...
    Aladin.prototype.createImageSurvey = function(id, name, rootUrl, cooFrame, maxOrder) {
        return new HpxImageSurvey(id, name, rootUrl, cooFrame, maxOrder);
    };
    ...
    Aladin.prototype.createSource = function(ra, dec, mesures) {
        return new cds.Source(ra, dec, mesures);
    };
