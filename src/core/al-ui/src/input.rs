...
pub fn canvas_element(runner_lock: &Gui) -> web_sys::HtmlCanvasElement {
    //&runner_lock.painter.canvas
...
/*pub fn canvas_element_or_die(canvas_id: &str) -> web_sys::HtmlCanvasElement {
    self::canvas_element(canvas_id)
        .unwrap_or_else(|| panic!("Failed to find canvas with id '{}'", canvas_id))
}*/

pub fn pos_from_mouse_event<'a>(runner_lock: &Gui, event: &web_sys::MouseEvent) -> egui::Pos2 {
    ...
    touch_for_pos
        ...
        .map_or(Default::default(), |touch| {
            *touch_id_for_pos = Some(egui::TouchId::from(touch.identifier()));
            pos_from_touch(canvas_origin(runner), &touch)
        })
}
...
fn push_touches(runner: &mut Gui, phase: egui::TouchPhase, event: &web_sys::TouchEvent) {
    let canvas_origin = canvas_origin(runner);
...
pub fn canvas_size_in_points(gui: &Gui) -> egui::Vec2 {
    let canvas = canvas_element(gui);
...
pub fn resize_canvas_to_screen_size(runner: &Gui, max_size_points: egui::Vec2) -> Option<()> {
    let canvas = canvas_element(runner);
...
pub fn handle_output(output: &egui::Output, runner: &mut Gui) {
    let egui::Output {
        cursor_icon,
        open_url,
        copied_text,
    ...
    if let Some(open) = open_url {
        self::open_url(&open.url, open.new_tab);
    }
...

    {
        if &runner.last_text_cursor_pos != text_cursor_pos {
            move_text_cursor(text_cursor_pos, &runner);
...
/*fn repaint_every_ms(runner_ref: &GuiRef, milliseconds: i32) -> Result<(), JsValue> {
...
fn manipulate_agent<'a>(
    ...
    let style = canvas_element(runner_lock).style();
...
fn move_text_cursor<'a>(cursor: &Option<egui::Pos2>, runner: &Gui) -> Option<()> {
