...
impl HpxTexture3D {
        ...
        match self.block_indices.binary_search(&block_idx) {
            Ok(_) => {
                if self.contains_slice(slice) {
                    ...
                    let m1 = 0xffff << (32 - slice_idx);
                    let m2 = 0xffff >> (slice_idx + 1);

                    let num_bits_to_side = (32 - slice_idx).min(slice_idx + 1) as u16;

                    let lb = ((block & m1) >> (32 - slice_idx)) as u32;
                    let rb = (block & m2) as u32;

                    let lb_trailing_zeros = lb.trailing_zeros() as u16;
                    let rb_leading_zeros = rb.leading_zeros() as u16;

                    match (
                        lb_trailing_zeros <= rb_leading_zeros,
                        lb_trailing_zeros <= num_bits_to_side,
                    ) {
                        (true, true) => Some(slice - lb_trailing_zeros - 1),
                        (false, false) => Some(slice + rb_leading_zeros + 1),
                        (true, false) => {
                            // One must explore the next block to see if a bit is set
                            if block_idx == self.blocks.len() - 1 {
                                ...
                                let num_bits_to_next_block =
                                    next_block.leading_zeros() as u16 + num_bits_to_side;
                        ...
                        (false, true) => {
                            if block_idx == 0 {
                                ...
                                let num_bits_from_prev_block =
                                    prev_block.trailing_zeros() as u16 + num_bits_to_side;
    ...
    pub fn extract_2d_slice_texture(&self, slice: u16) -> Option<HpxTexture2D> {
        ...
        if self.blocks[block_idx] & (1 << (31 - slice_idx)) == 1 {
    ...
    pub fn contains_slice(&self, slice: u16) -> bool {
        ...
        self.blocks[block_idx] & (1 << (31 - slice_idx)) == 1
    }
}
