    ...
    pub fn draw<P: Projection>(
        ...
        if self.enabled {
            ...
            if !self.hide_labels {
                ...
                self.ctx2d.set_font(&format!("{}px Verdana", text_height as usize));
...
impl Label {
    fn meridian<P: Projection>(fov: &FieldOfViewType, lon: f64, m1: &Vector3<f64>, camera: &CameraViewPort, sp: Option<&Vector2<f64>>, ctx2d: &CanvasRenderingContext2d) -> Option<Self> {
        let LonLatT(_, lat) = &camera.get_center().lonlat();
        ...
        let s2 = P::model_to_screen_space(&m2.extend(1.0), camera)?;
        ...
        let rot = if ds.y > 0.0 { ds.x.acos() } else { -ds.x.acos() };
        let rot = if ds.y > 0.0 {
            if rot > HALF_PI {
                -PI + rot
            } else {
    ...
    fn parallel<P: Projection>(fov: &FieldOfViewType, lat: f64, m1: &Vector3<f64>, camera: &CameraViewPort, ctx2d: &CanvasRenderingContext2d) -> Option<Self> {
    ...
    fn size(camera: &CameraViewPort) -> f64 {
        ...
        let ndc1 = crate::renderable::projection::clip_to_ndc_space(&Vector2::new(-1.0, 0.0), camera);
        let ndc2 = crate::renderable::projection::clip_to_ndc_space(&Vector2::new(1.0, 0.0), camera);

        let dx = ndc2.x - ndc1.x;
        let allsky = dx < 2.0;

        let ss = camera.get_screen_size();
        let ds = (ss.x*ss.x + ss.y*ss.y).sqrt();
...
impl GridLine {
    fn meridian<P: Projection>(
        ...
        let fov = camera.get_field_of_view();
        let p = fov.intersect_meridian(Rad(lon), camera)?;
...
fn lines<P: Projection>(
    ...
    while theta < stop_theta {
        if let Some(line) = GridLine::meridian::<P>(ctx2d, theta, &bbox.get_lat(), sp.as_ref(), camera) {
            lines.push(line);
        }
        theta += step_lon;
    }
