...
use std::ops::{Div, Add, Sub, Mul, AddAssign, MulAssign, SubAssign, RemAssign, DivAssign};
...
impl Projection for Mollweide {
    ...
    fn clip_to_world_space(
        ...
        if Self::is_included_inside_projection(&pos_clip_space) {
            ...
            let delta = ((2.0 * (2.0 * pos_clip_space.y).asin()
                + 4.0 * pos_clip_space.y * k)
                / f64::PI())
                .asin();
...
impl Projection for Orthographic {
    ...
    fn clip_to_world_space(
        ...
        let xw_2 =
            1.0 - pos_clip_space.x * pos_clip_space.x - pos_clip_space.y * pos_clip_space.y;
        if xw_2 > 0.0 {
